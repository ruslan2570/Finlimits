...
@section('content')
    ...
    <form method="post" action="{{ route('auth.login') }}" class="max-w-md mx-auto space-y-6">
        @csrf
        <div class="space-y-4">
            <div>
        ...
        <div class="space-y-4">
            ...
            <div class="flex items-center justify-between">
                <a href="{{ route('auth.restore.show') }}" class="text-sm ">
                    Забыли пароль?
                </a>
                <a href="{{ route('auth.registration.show') }}" class="text-sm text-gray-600 hover:text-gray-800">
                    Зарегистрироваться
                </a>
            </div>
        </div>
